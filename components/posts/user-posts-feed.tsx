...
export const UserPostsFeed: FC<UserPostsFeedProps> = ({ session, userId }) => {
  ...
  useEffect(() => {
    const fetchMorePosts = async () => {
      if (dontFetch) return;
      try {
        const initialFetchLink = `/api/posts?author=${userId}&sk=${skip}&tk=${initialFetchAmount}`;
        const fetchLink = `/api/posts?author=${userId}sk=${
